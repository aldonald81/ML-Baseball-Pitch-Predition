...
def main():
    ...
    if knn:
        ...
        # Find avg k value and f1 score over 100 runs
        """
        k = 0
        score = 0
        for i in range(100):
            score_run, k_run = kNeighbors(data, columns)
            k+= k_run
            score += score_run
        k/=100
        score/=100
        ...
        print(k) # 10.62
        print(score) # 0.8549952483556218
        """
        ...
        SVM(train_x, train_y, test_x, test_y)
         ...
         # Finding best params over 10 runs
        """
        C = 0
        tol = 0
        score = 0
        for i in range(10):
            score_run, C_run, tol_run = SVM(train_x, train_y, test_x, test_y)
            C+= C_run
            tol += tol_run
            score += score_run
        C/=10
        ...
        print(C) # 2.387496157029248
        print(tol) # 0.0006918202991034834
        print(score) # 0.8490362757268362
        """
